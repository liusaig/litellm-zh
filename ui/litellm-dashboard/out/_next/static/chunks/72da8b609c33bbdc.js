...
client = openai.OpenAI(
	api_key="${v||"YOUR_LITELLM_API_KEY"}",
	base_url="${y}"
)`;switch(h){case a.CHAT:{let e=Object.keys(k).length>0,r="";if(e){let e=JSON.stringify({metadata:k},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();r=`,
    extra_body=${e}`}let o=C.length>0?C:[{role:"user",content:I}];t=`
import base64
